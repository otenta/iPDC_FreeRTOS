...
int dataparser(unsigned char data[]) {
    ...
    if (match) {
        ...
        df = (struct data_frame *)malloc(sizeof(struct data_frame));
        if (!df) {
        ...
        df->framesize = (unsigned char *)malloc(3 * sizeof(unsigned char));
        if (!df->framesize) {
        ...
        df->idcode = (unsigned char *)malloc(3 * sizeof(unsigned char));
        if (!df->idcode) {
            printf("Not enough memory for df->idcode\n");
            exit(1);
        }
        ...
        df->soc = (unsigned char *)malloc(5 * sizeof(unsigned char));
        if (!df->soc) {
        ...
        df->fracsec = (unsigned char *)malloc(5 * sizeof(unsigned char));
        if (!df->fracsec) {
        ...
        df->dpmu = (struct data_for_each_pmu **)malloc(num_pmu * sizeof(struct data_for_each_pmu *));
        if (!df->dpmu)
        {
        ...
        for (i = 0; i < num_pmu; i++)
        {
            df->dpmu[i] = (struct data_for_each_pmu *)malloc(sizeof(struct data_for_each_pmu));
        }
        ...
        while (j < num_pmu)
        {
            ...
            if (stat_status == 16)
            {
                df->dpmu[j]->stat = (unsigned char *)malloc(3 * sizeof(unsigned char));
                if (!df->dpmu[j]->stat)
                {
            ...
            // Phasors
            df->dpmu[j]->phasors = (unsigned char **)malloc(phnmr * sizeof(unsigned char *));
            ...
            for (i = 0; i < phnmr; i++)
            {
                ...
                {
                    df->dpmu[j]->phasors[i] = (unsigned char *)malloc(9 * sizeof(unsigned char));
                }
                else
                {
                    df->dpmu[j]->phasors[i] = (unsigned char *)malloc(5 * sizeof(unsigned char));
                }
            }

            // Analogs
            df->dpmu[j]->analog = (unsigned char **)malloc(annmr * sizeof(unsigned char *));
            if (!df->dpmu[j]->analog)
            {
            ...
            for (i = 0; i < annmr; i++)
            {
                ...
                {
                    df->dpmu[j]->analog[i] = (unsigned char *)malloc(9 * sizeof(unsigned char));
                }
                else
                {
                    df->dpmu[j]->analog[i] = (unsigned char *)malloc(5 * sizeof(unsigned char));
                }
            }

            // Frequency
            df->dpmu[j]->freq = (unsigned char *)malloc(5 * sizeof(unsigned char));
            df->dpmu[j]->dfreq = (unsigned char *)malloc(5 * sizeof(unsigned char));

            // Digital
            df->dpmu[j]->digital = (unsigned char **)malloc(dgnmr * sizeof(unsigned char *));
            ...
            for (i = 0; i < dgnmr; i++)
            {
                df->dpmu[j]->digital[i] = (unsigned char *)malloc(3 * sizeof(unsigned char));
            }
            ...
            // Copy FMT
            df->dpmu[j]->fmt = (struct format *)malloc(sizeof(struct format));
